<x-app-layout>
    <x-slot name="header">
        @lang('crud.ssh_accesses.index_title')    </x-slot>
            ...
            <div class="block w-full overflow-auto scrolling-touch">
                <table class="w-full text-sm text-left rtl:text-right text-gray-500">
                    ...
                    <tbody class="text-gray-600">
                        @forelse($sshAccesses as $sshAccess)
                        <tr class="hover:bg-gray-50 {{ $loop->last ? '' : 'border-b' }} {{ $loop->index % 2 !== 0 ? 'bg-gray-50' : '' }}">
                            ...
                            >
                                <div
                                    ...
                                    "
                                >
                                    @can('update', $sshAccess)
                                    <a
                                        href="{{ route('ssh-accesses.edit', $sshAccess) }}"
                                        class="mr-1"
                                    >
                                        <button
                                            type="button"
                                    ...
                                    @endcan @can('view', $sshAccess)
                                    <a
                                        href="{{ route('ssh-accesses.show', $sshAccess) }}"
                                        class="mr-1"
                                    >
                                        <button
                                            type="button"
                                    ...
                                    @endcan @can('delete', $sshAccess)
                                    <form
                                        action="{{ route('ssh-accesses.destroy', $sshAccess) }}"
                                        method="POST"
